...
def create_video_from_images(image_folder, video_name, fps=30):
    ...
    image_files = [img for img in os.listdir(image_folder) if img.endswith(".png") and 'concat_relight' in img]
...
if __name__ == '__main__':
    ...
    for scene_name in os.listdir(data_root):
        ...
        for frame_idx in frame_list:
            images = []
            img_0_sample = cv2.imread(os.path.join(scene_save_path, f'{frame_idx}_0_{txt}.png'))
            img_height, img_width, _ = img_0_sample.shape
            for cam_id in [1,0,2]:
                img = cv2.imread(os.path.join(scene_save_path, f'{frame_idx}_{cam_id}_{txt}.png'))
                images.append(img)
            ...
            blank_img[:] = white
            images = []
            img = cv2.imread(os.path.join(scene_save_path, f'{frame_idx}_3_{txt}.png'))
            images.append(img)
            images.append(blank_img)
            img = cv2.imread(os.path.join(scene_save_path, f'{frame_idx}_4_{txt}.png'))
            images.append(img)
