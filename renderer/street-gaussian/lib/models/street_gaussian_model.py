...
class StreetGaussianModel(nn.Module):
    def __init__(self, metadata):
        ...
        self.frame_vehicle_data = metadata['frame_vehicle_data']
        self.agents_id = metadata['agents_id']
    ...
    def load_state_dict(self, state_dict, exclude_list=[]):
        for model_name in self.model_name_id.keys():
            ...
            if model_name in state_dict:
                print('Loading model in the state dict: ', model_name)
    ...
    def setup_functions(self):
        ...
        if self.include_obj:
            for track_id, obj_meta in self.obj_info.items():
                ...
                # ego_trans_x = ego_pose[3, 0]
                # ego_pose_rot = matrix_to_quaternion(ego_pose[:3, :3].unsqueeze(0)).squeeze(0)
                # obj_rot_q = obj_tracklets[4:8]
                # ego_forward = self.quaternion_to_direction(ego_pose_rot)
                # obj_forward = self.quaternion_to_direction(obj_rot_q)
                # dot_product = np.dot(ego_forward, obj_forward)
                if track_id in self.agents_id:
                    ...
                    # model.create_from_pcd(spatial_lr_scale=1)
                    model.load_custom_ply()
    ...
    def parse_camera(self, camera: Camera):
        ...
        if self.include_obj:
            ...
            frame_idx = camera.render_frame_idx
            for i, obj_name in enumerate(self.obj_list):
                ...
                if self.frame_vehicle_data[frame_idx][track_id]['visible'] == 1:
        ...
        if len(self.graph_obj_list) > 0:
            ...
            for i, obj_name in enumerate(self.graph_obj_list):
                ...
                if track_id not in self.metadata['agents_id']:
                    obj_rot = self.actor_pose.get_tracking_rotation(track_id, self.viewpoint_camera)
                    obj_trans = self.actor_pose.get_tracking_translation(track_id, self.viewpoint_camera)                
                    ego_pose = self.viewpoint_camera.ego_pose
                    ego_pose_rot = matrix_to_quaternion(ego_pose[:3, :3].unsqueeze(0)).squeeze(0)
                    ...
                    # frame_idx = self.viewpoint_camera.meta['frame_idx']
                    # try:
                    #     theta = self.frame_vehicle_data[frame_idx][track_id]['rot']
                    # except KeyError:
                    #     print(f"KeyError: {track_id} at frame index {frame_idx}, skipping")
                    #     continue
                    obj_rot = self.actor_pose.get_agent_rotation(track_id, self.viewpoint_camera, self.frame_vehicle_data)
                    obj_trans = self.actor_pose.get_agent_translation(track_id, self.viewpoint_camera, self.frame_vehicle_data)                
                    ego_pose = self.viewpoint_camera.ego_pose
                    ego_pose_rot = matrix_to_quaternion(ego_pose[:3, :3].unsqueeze(0)).squeeze(0)
                    obj_rot_tensor = torch.from_numpy(obj_rot).to(ego_pose_rot.device)
                    obj_trans_tensor = torch.from_numpy(obj_trans).to(ego_pose_rot.device)
                    obj_trans_tensor = obj_trans_tensor.float()
                    obj_rot_q= matrix_to_quaternion(obj_rot_tensor.unsqueeze(0)).squeeze(0)
                    obj_rot = obj_rot_q.expand(obj_model.get_xyz.shape[0], -1)
                    obj_trans = obj_trans_tensor.unsqueeze(0).expand(obj_model.get_xyz.shape[0], -1)
                    # print(obj_trans)
                    self.obj_rots.append(obj_rot)
                    self.obj_trans.append(obj_trans)                    
            ...
            for obj_name in self.graph_obj_list:
                ...
                if obj_model.deformable or self.flip_prob == 0:
                    # print(obj_name, obj_model.get_xyz.shape)
    ...
    def get_opacity(self):
        ...
        for obj_name in self.graph_obj_list:
            ...
            # opacity_new = torch.ones_like(opacity) * 0.3
            # # opacity_new = torch.ones_like(opacity)
