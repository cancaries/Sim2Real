...
class StreetGaussianRenderer():
    ...
    def render_all(
        ...
        result['rgb_background_wo_sky'] = render_background['rgb_wo_sky']
    ...
    def render_background(
        ...
        if pc.use_color_correction:
            ...
            result['rgb_wo_sky'] = pc.color_correction(viewpoint_camera, result['rgb_wo_sky'])

        if cfg.mode != 'train':
            result['rgb'] = torch.clamp(result['rgb'], 0., 1.)
            result['rgb_wo_sky'] = torch.clamp(result['rgb_wo_sky'], 0., 1.)
