...
class StreetGaussianVisualizer():
    ...
    def visualize(self, result, camera: Camera):
        ...
        rgb_bkgd_wo_sky = result['rgb_background_wo_sky']
        ...
        if self.save_image:
            ...
            torchvision.utils.save_image(rgb_bkgd_wo_sky, os.path.join(self.result_dir, f'{name}_rgb_bkgd_wo_sky.png'))
    ...
    def visualize_novel_view(self, result, camera: Camera):
        ...
        if self.save_image:
            ...
            imageio.imwrite(os.path.join(self.result_dir, f'{id:06d}_depth.png'), depth)
    ...
    def visualize_depth(self, result, camera: Camera):
        ...
        if self.save_image:
            ...
            # imageio.imwrite(os.path.join(self.result_dir, f'{name}_depth.png'), depth)
            # 存储保留原始信息的深度图
            np.save(os.path.join(self.result_dir, f'{name}_depth.npy'), depth)
    ...
    def save_video_from_frames(self, frames, name, visualize_func=None):
        ...
        if len(unqiue_cams) == 1:
            ...
            if len(concat_cameras) == len(unqiue_cams):
                ...
                # print('frames_cam_len:', frames_cam_len)
