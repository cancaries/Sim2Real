    ...
    with torch.no_grad():
        ...
        for idx, cur_ego_pose in enumerate(tqdm(ego_poses, desc="Rendering Trajectory")):
            ...
            for num_cam in range(len(concat_cameras)):
                ...
                #     modified_ego_pose = rotate_matrix(cur_ego_pose, 0.0, pitch_deg=0)
                # elif cam_id == 1:
                #     # 向左旋转 25.2°
                #     modified_ego_pose = rotate_matrix(cur_ego_pose, 45.0, pitch_deg=0)
                # elif cam_id == 2:
                #     # 向右旋转 25.2°
                #     modified_ego_pose = rotate_matrix(cur_ego_pose, -45.0, pitch_deg=0)
                # elif cam_id == 3:
                #     modified_ego_pose = rotate_matrix(cur_ego_pose, 90.0, pitch_deg=7.5, roll_deg=0)
