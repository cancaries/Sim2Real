...
class Map:
        ...
        for i, waypoint in enumerate(plan_waypoints):
            ...
            if i == 0:
                ...
                if waypoint.road_option != RoadOption.STRAIGHT:
                    ...
                    distance = calculate_distance(plan_waypoints_new[-1].transform.location, waypoint.transform.location)
    ...
    def generate_overall_plan_waypoints(self, current_location, driving_mode='Random', ignore_lanechange=True, max_plan_length=5000, min_plan_length=200):
        ...
        while True:
            if flag_first_plan:
                ...
                driving_mode, new_plan = self.get_plan_waypoints(plan_waypoints[-1].transform.location, driving_mode='Random',ignore_lanechange=ignore_lanechange)
                if new_plan is None:
                    break
    ...
    def generate_waypoint_path_from_transform_path(self, transform_path):
        ...
        for idx, transform in enumerate(transform_path):
            ...
            next_lane_id, next_point_index, _ = self.find_nearest_lane_point(transform_path[idx+1].location)
    ...
    def generate_waypoint_path_from_two_points(self, cur_waypoint, next_waypoint, direction='CHANGELANERIGHT'):
        ...
        first_rotation = calculate_rotation(cur_waypoint.transform.location, self.features[next_lane_id].polyline[next_point_index]) 
        plan_waypoints = []
        first_waypoint_config = self.build_waypoint_config(cur_waypoint.transform.location, first_rotation, roadoption, cur_lane_id, cur_point_index)
        first_waypoint = Waypoint(first_waypoint_config)
        plan_waypoints.append(first_waypoint)
        next_waypoint_config = self.build_waypoint_config(next_waypoint.transform.location, next_waypoint.transform.rotation, RoadOption.STRAIGHT, next_lane_id, next_point_index)
    ...
    def get_waypoint_w_offset(self, waypoint, offset=1.0, direction=None):
        ...
        yaw = waypoint.transform.rotation[2]
        ...
        waypoint_new = Waypoint(self.build_waypoint_config([x, y, location[2]], waypoint.transform.rotation, waypoint.road_option, waypoint.lane_id, waypoint.lane_point_idx))
        return waypoint_new
    ...
    def generate_overtake_path_from_reference_path(self, reference_path,direction='left',overtake_offset=1.5):
        ...
        next_location_w_offset = calculate_point_w_offset(next_waypoint.transform.location, next_waypoint.transform.rotation[2], overtake_offset, direction)
        location = first_waypoint.transform.location
        ...
        for idx,cur_waypoint in enumerate(reference_points):
            ...
            if idx == 0:
                continue
            cur_location_w_offset = calculate_point_w_offset(cur_waypoint.transform.location, cur_waypoint.transform.rotation[2],overtake_offset, direction)
            next_location_w_offset = calculate_point_w_offset(reference_points[idx+1].transform.location, reference_points[idx+1].transform.rotation[2], overtake_offset, direction)
        ...
        last_2_location_w_offset = calculate_point_w_offset(last_2_waypoint.transform.location, last_2_waypoint.transform.rotation[2],overtake_offset, direction)
        last_2_rotation = calculate_rotation(last_2_location_w_offset, last_waypoint.transform.location)
    ...
    def draw_map_w_waypoints(self, waypoints, file_path=None, spawn_point_num=None):
        ...
        for lane in lanes:
            ...
            plt.plot(lane[:,0],lane[:,1],c='green') 
        # 起点标绿
        plt.scatter(waypoints[0].transform.location[0], waypoints[0].transform.location[1], c='green', s=100)
        # 画上起点的方向
        # rotation = waypoints[0].transform.rotation
        # rotation = rotation[2]
        # x = waypoints[0].transform.location[0]
        # y = waypoints[0].transform.location[1]
        # dx = 0.5 * math.cos(rotation)
        # dy = 0.5 * math.sin(rotation)
        # plt.arrow(x, y, dx, dy, head_width=2, head_length=2, fc='k', ec='k')
        # # 终点标红
        # plt.scatter(waypoints[-1].transform.location[0], waypoints[-1].transform.location[1], c='red', s=100)
        # plt.text(waypoints[0].transform.location[0], waypoints[0].transform.location[1], f"Spawn Point Num: {spawn_point_num}", fontsize=20)
        for waypoint in waypoints:
            # waypoint = np.array(waypoint.transform.location)
            # plt.scatter(waypoint[0], waypoint[1], c='blue', s=10)
            # 画上方向
            rotation = waypoint.transform.rotation
            rotation = rotation[2]
            x = waypoint.transform.location[0]
            y = waypoint.transform.location[1]
            dx = 0.5 * math.cos(rotation)
            dy = 0.5 * math.sin(rotation)
            plt.arrow(x, y, dx, dy, head_width=2, head_length=2, fc='k', ec='k')
    ...
    def draw_map_w_key_waypoints(self, waypoints, spawn_point_num=None):
        ...
        plt.text(waypoints[0].transform.location[0], waypoints[0].transform.location[1], f"Spawn Point Num: {spawn_point_num}", fontsize=20)
        for waypoint in waypoints:
            waypoint_loc = np.array(waypoint.transform.location)
            if waypoint.road_option != RoadOption.STRAIGHT:
...
if __name__ == '__main__':
    map_json_path = '/home/ubuntu/junhaoge/real_world_simulation/data/waymo_multi_view/031/map_feature.json'
    # 加载JSON数据
