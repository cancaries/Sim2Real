...
for scene_name in ['003','002','007','015','016','021','019','021','025','022','031','036']:
    lines = []
    map_feature_path = f'/home/ubuntu/junhaoge/real_world_simulation/data/waymo_multi_view/{scene_name}/map_feature.json'
    ...
    # output_lane = []
    # not_closed_idx = []
    # for line_idx, line in enumerate(final_lines):
    #     if not is_closed(line, closure_threshold):
    #         not_closed_idx.append(line_idx)
    #         continue
    # if len(not_closed_idx) == 1:
    #     start = final_lines[not_closed_idx[0]][0]
    #     end = final_lines[not_closed_idx[0]][-1]
    #     # 如果起点和终点足够近，则闭合并进行插值
    #     interp_points = interpolate_between_points(start, end, 10)
    #     line = np.concatenate((final_lines[not_closed_idx[0]], interp_points), axis=0)  # 闭合
    #     final_lines[not_closed_idx[0]] = line
    # output_lane = final_lines
    # 绘制最终合并后的曲线
    ...
    # 以浅蓝色画出final_lines线段所围成面积最大的contours，其他的contours用浅灰色画出
