...
def post_process_single_frame(folder, frame):
    # iclight
    for cam_id in range(3):
        Parallel(n_jobs=1)(delayed(iclight_single_image)(os.path.join(folder,str(frame)+'_'+str(cam_id) +'_rgb.png'),\
                                                        os.path.join(folder,str(frame)+'_'+str(cam_id)+'_rgb_iclight.png')))
...
if __name__ == '__main__':
    ...
    frame_list.sort()
    # 遍历 frame_list 并调用 post_process_single_frame
    for frame in frame_list:
        post_process_single_frame(input_path, frame)
